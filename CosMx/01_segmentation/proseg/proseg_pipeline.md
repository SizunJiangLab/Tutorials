...
Proseg (**pro**babilistic **seg**mentation) is a cell segmentation method for in situ spatial transcriptomics. This pipeline is to help you set up the environment for CosMx segmentation using proseg. 
...
## 1. Fix the bug regarding input files in `stitch-cosmx.jl`
...
The latest file structure of the AtoMx outfile is like:
...
Because the output file structure of AtoMx is messy, it is easy to input the long and messy diretory incorrectly. Morevoer, we need to fix the bug regarding input files in `stitch-cosmx.jl`. So I create an executable shell script to make the proseg convenient to run. 
