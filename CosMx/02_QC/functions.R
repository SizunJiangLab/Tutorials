...
plt_negative_adjust_Fov <- function(negative_adjust, plot.by) {
  y_lab <- c(
    "division" = "(True Gene + 1) / (Negative Control + 1)",
    "subtract" = "True Gene - Negative Control"
  )[plot.by]
  p <- negative_adjust %>%
    select(setNames(
      c("fov", plot.by),
...
plt_TranscriptsPerCellPerFOVCt <- function(cell_comp_summary = cell_comp_summary, ylim_ranges = list(NULL, NULL, NULL, NULL)) {
  ...
  # plot
  p0 <- ggplot(cell_comp_summary, aes(x = fov, y = count_None)) +
    theme_minimal() +
    geom_boxplot(width = 0.5 * length(unique(cell_comp_summary$fov)) / length(unique(cell_comp_summary$fov))) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    ylim(ylim_ranges[[1]]) +
  ...
  p1 <- ggplot(cell_comp_summary, aes(x = fov, y = count_Membrane)) +
    theme_minimal() +
    geom_boxplot(width = 0.5 * length(unique(cell_comp_summary$fov)) / length(unique(cell_comp_summary$fov))) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    ylim(ylim_ranges[[2]]) +
  ...
  p2 <- ggplot(cell_comp_summary, aes(x = fov, y = count_Nuclear)) +
    theme_minimal() +
    geom_boxplot(width = 0.5 * length(unique(cell_comp_summary$fov)) / length(unique(cell_comp_summary$fov))) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    ylim(ylim_ranges[[3]]) +
  ...
  p3 <- ggplot(cell_comp_summary, aes(x = fov, y = count_Cytoplasm)) +
    theme_minimal() +
    geom_boxplot(width = 0.5 * length(unique(cell_comp_summary$fov)) / length(unique(cell_comp_summary$fov))) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    ylim(ylim_ranges[[3]]) +
...
plt_UnassignedNegativeProbePerFov <- function(combined_data, ylim_range = NULL) {
  ...
  p <- ggplot(NegativeProbePerCellPerFOV, aes(x = fov, y = NegativeProbeCounts)) +
    theme_minimal() +
    geom_col() +
    labs(y = "Negative Probe counts") +
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
plt_NegativeProbePerCellPerFov <- function(combined_data, ylim_range = NULL) {
  ...
  p <- ggplot(expression_counts, aes(x = fov, y = Negative_expressed)) +
    theme_minimal() +
    geom_boxplot() +
    labs(y = "Negative Probe counts") +
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
plt_DensityTranscripts <- function(combined_data, max_density) {
  p <- ggplot(combined_data$tx_sub, aes(x = x_global_px, y = y_global_px)) +
    ...
    labs(
      x = "X Global Position (px)", y = "Y Global Position (px)",
