...
path_flat_files <- c(
  "/mnt/nfs/storage/CosMX/DLBCL_TMA001_section04_v132/AtoMx/flatFiles/HL_DLBCL_TMA1_BOSTON_1ug/", 
  "/mnt/nfs/storage/CosMX/DLBCL_TMA001_section06_v132/AtoMx/flatFiles/HL_DLBCL_TMA1_BOSTON_2ug/"
)
path_logs_files <- c(
  "/mnt/nfs/storage/CosMX/DLBCL_TMA001_section04_v132/AtoMx/RawFiles/HL_DLBCL_TMA1_BOSTON_1ug/Logs/", 
  "/mnt/nfs/storage/CosMX/DLBCL_TMA001_section06_v132/AtoMx/RawFiles/HL_DLBCL_TMA1_BOSTON_2ug/Logs/"
)
...
# Limit of Detection (LOD) for Negative Probes (per FOV)

## 1 sigma

### Unique Transcripts {.tabset .tabset-pills}
```{r, results='asis'}
list_lod <- list_mean_sd_type %>% 
    map(~ .x %>% mutate(cut = mean + 1 * sd) %>% filter(group.by == "Negative") %>% select(fov, cut))
list_output <- list_combined_data %>% 
  map(~ .x %>% plt_n_unique_Fov())
fill_range <- get_pdata_range(list_output, "n", 1, vmin = 1, vmax = NULL) %>% log10()
list_output <- map2(
  ...
  list_lod, 
  ~ {
    .x$data <- .x$data %>% left_join(.y, by = "fov") %>% mutate(n = floor(n - cut)) %>% filter(n > 0)
    return(.x)
  }
)
list_output <- list_output %>% 
  map(
    ~ .x + 
      scale_fill_continuous(type = "viridis", limits = fill_range) +
      scale_x_discrete(labels = ~ .x %>% str_pad(width = nchar_fov, pad = "0")) +
      scale_y_discrete(breaks = target)
  )
...
### Calls Above Negative Probe LOD {.tabset .tabset-pills}
```{r, results='asis'}
list_output <- map2(
  list_combined_data, 
  list_lod, 
  ~ plt_percent_above_lod(combine_data = .x, lod = .y)
)
list_output <- list_output %>% 
  map(
    ~ .x +
...
```{r, results='asis'}
list_lod <- list_mean_sd_type %>% 
    map(~ .x %>% mutate(cut = mean + 2 * sd) %>% filter(group.by == "Negative") %>% select(fov, cut))
list_output <- list_combined_data %>% 
  map(~ .x %>% plt_n_unique_Fov())
fill_range <- get_pdata_range(list_output, "n", 1, vmin = 1, vmax = NULL) %>% log10()
list_output <- map2(
...
```{r, results='asis'}
list_lod <- list_mean_sd_type %>% 
    map(~ .x %>% mutate(cut = mean + 3 * sd) %>% filter(group.by == "Negative") %>% select(fov, cut))
list_output <- list_combined_data %>% 
  map(~ .x %>% plt_n_unique_Fov())
fill_range <- get_pdata_range(list_output, "n", 1, vmin = 1, vmax = NULL) %>% log10()
list_output <- map2(
